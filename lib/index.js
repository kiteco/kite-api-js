...
const NodeClient = require('kite-connect/lib/clients/node');
...
const KiteAPI = {
  ...
  getReportDataAtPosition(filename, source, position) {
    checkArguments(this.getReportDataAtPosition, filename, source, position);
  ...
  isFileAuthorized(filename) {
    checkArguments(this.isFileAuthorized, filename);
    return this.request({
  ...
  Account: {
    client: new NodeClient('alpha.kite.com', -1),
...
const checkStatusAndInvokeCallback = callback => resp => {
  ...
  if (resp.statusCode !== 200) {
    throw new Error(`Invalid status ${resp.statusCode}`);
  }
  return resp;
};
