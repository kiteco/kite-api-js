...
const KiteAPI = {
  ...
  getSnippetCompletionsAtPosition(filename, source, start_position, editor) {
    checkArguments(this.getSnippetCompletionsAtPosition, filename, source, start_position, editor);
    if (source.length > MAX_FILE_SIZE) { return Promise.resolve([]); }
    const payload = {
      text: source,
      editor,
