...
const KiteAPI = {
  ...
  validateFileForRelatedCode(filename) {
    return this.request(
      {
        path: '/codenav/validate',
        method: 'POST',
      },
  ...
  requestRelatedCode(editor, filename, line, selection) {
    return this.request(
      {
        path: '/clientapi/editor/related',
        method: 'POST',
      },
      JSON.stringify({ editor, filename, line, selection })
    );
  },
